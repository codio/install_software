...
EXISTS=$(jq -r --arg val "$LAST_20_CHARS" '.customApiResponses.rejected[]? | select(. == $val)' "$JSON_FILE" 2>/dev/null)

if [ -n "$EXISTS" ]; then
  exit 0
fi

# Add the value to the customApiResponses.rejected array
# This will create the nested structure if it doesn't exist
jq --arg val "$LAST_20_CHARS" '
    .customApiResponses.rejected //= [] | 
    .customApiResponses.rejected += [$val]
